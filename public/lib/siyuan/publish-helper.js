...
const initPublishHelper = () => {
   ...
   */
  window.syp.renderPublishHelper = (pageId, pageUrl, mainWin = window, isDev = false) => {
    ...
    new Promise(function (html) {
      ...
      fs.readFile(`${window.siyuan.config.system.dataDir}/widgets/sy-post-publisher/${url}`, function (err, data) {
        if (err) {
          console.log(err)
        }
