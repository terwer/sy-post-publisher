...
export class MetaWeblogApi {
  ...
  public async getUsersBlogs(appkey: string, username: string, password: string): Promise<UserBlog[]> {
    const usersBlogs: UserBlog[] = []

    const ret = await this.commonXmlrpcClient.methodCall(METAWEBLOG_METHOD_CONSTANTS.GET_USERS_BLOGS, [
      this.apiType,
      username,
      password,
    ])
  ...
  public async getRecentPosts(appkey: string, username: string, password: string, numOfPosts: number): Promise<Post[]> {
    return []
  }
  ...
  public async getPost(postid: string, username: string, password: string): Promise<Post> {
    ...
    try {
      const ret = await this.commonXmlrpcClient.methodCall(METAWEBLOG_METHOD_CONSTANTS.GET_POST, [
        postid,
        username,
        password,
      ])
  ...
  public async newPost(
    ...
    let ret = await this.commonXmlrpcClient.methodCall(METAWEBLOG_METHOD_CONSTANTS.NEW_POST, [
      this.apiType,
      username,
      password,
      postStruct,
  ...
  public async editPost(
    ...
    const postStruct = this.createPostStruct(post)
    this.logger.debug("postStruct=>", postStruct)
    const ret = await this.commonXmlrpcClient.methodCall(METAWEBLOG_METHOD_CONSTANTS.EDIT_POST, [
      postid,
      username,
      password,
      postStruct,
  ...
  public async deletePost(
    ...
    const ret = await this.commonXmlrpcClient.methodCall(METAWEBLOG_METHOD_CONSTANTS.DELETE_POST, [
      appKey,
      postid,
      username,
      password,
  ...
  public async getCategories(blogid: string, username: string, password: string): Promise<CategoryInfo[]> {
    ...
    try {
      const ret = await this.commonXmlrpcClient.methodCall(METAWEBLOG_METHOD_CONSTANTS.GET_CATEGORIES, [
        this.apiType,
        username,
        password,
      ])
