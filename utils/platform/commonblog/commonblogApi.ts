...
import { isElectron, isInChromeExtension, isLocalhost } from "~/utils/browserUtil"
...
export class CommonblogApi {
   ...
   * @param formJson 可选，发送form请求才需要
   */
  private async fetchChrome(apiUrl: string, fetchOptions: RequestInit, formJson?: any[]): Promise<string> {
    try {
      const reqOps = {
   ...
   * @param formJson 可选，发送form请求才需要
   */
  private async fetchCall(apiUrl: string, fetchOptions: RequestInit, formJson?: any[]): Promise<any> {
    let result
