  ...
  <div class="preference-setting">
    <el-form label-width="150px" inline>
      <el-form-item :label="$t('preference.setting.fixTitle')">
        <el-switch v-model="formData.fixTitle" @change="formMethods.onTitleChange" />
      </el-form-item>
      <el-form-item :label="$t('preference.setting.removeH1')">
        <el-switch v-model="formData.removeH1" @change="formMethods.onH1Change" />
      </el-form-item>
      <el-form-item :label="$t('preference.setting.newWin')">
        <el-switch v-model="formData.newWin" @change="formMethods.onNewWinChange" />
      </el-form-item>
      <el-form-item :label="$t('main.auto.fetch.tag')">
        <el-switch v-model="formData.autoTag" @change="formMethods.onAutoTagChange" />
      </el-form-item>
      <el-form-item v-if="false" :label="$t('siyuan.browser.show.close.btn')">
        <el-switch v-model="formData.showCloseBtn" @change="formMethods.onShowCloseBtnChange" />
      </el-form-item>
      <el-form-item :label="$t('github.post.picgo.use')">
        <el-switch v-model="formData.usePicgo" @change="formMethods.onUsePicgoBtnChange" />
      </el-form-item>
    </el-form>
    ...
    <el-form label-width="150px" v-if="false">
      ...
      <el-form-item :label="$t('setting.main.background')">
        ...
        <el-alert :title="$t('setting.main.background.tip')" type="warning" :closable="false" />
      </el-form-item>
      <el-form-item>
        <el-button @click="formMethods.handleSavePreference">{{ $t("main.opt.ok") }} </el-button>
      </el-form-item>
    </el-form>
  </div>
</template>
...
const logger = LogFactory.getLogger("components/publish/tab/preference/PreferenceSetting.vue")
const { t } = useI18n()
