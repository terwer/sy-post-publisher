...
<template>
  <el-form class="switch-form" inline label-width="150px">
    ...
    <!-- 动态配置 -->
    <el-form-item v-for="cfg in switchFormData.dynamicConfigArray" :key="cfg.posid" :label="cfg.platformName">
      <el-switch
    ...
    <div v-if="showSwitchTip">
      <p>
        <el-alert :title="$t('platform.must.select.one')" type="error" :closable="false" />
      </p>
    </div>
  </el-form>
</template>
...
import { getDynSwitchActive, getDynSwitchInactive, getSwitchItem } from "~/utils/platform/dynamicConfig"
