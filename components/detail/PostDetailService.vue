...
<template>
  <div class="post-detail-wrap">
    ...
    <el-alert class="top-share-tip" :title="$t('siyuan.local.share.tip')" type="warning" :closable="false" />
    <blockquote class="post-detail-id">
      <span class="id-text">本文ID：{{ post.postid }}</span>

      <!-- 复制ID -->
      <el-tooltip class="box-item" effect="dark" :content="$t('post.detail.button.copy.id')" placement="bottom-start">
      ...
      <!-- 管理Anki -->
      <el-tooltip class="box-item" effect="dark" :content="$t('post.detail.button.anki.mark')" placement="bottom-start">
      ...
      <!-- 文章绑定 -->
      <el-tooltip class="box-item" effect="dark" :content="$t('service.tab.post.bind')" placement="bottom-start">
        <el-button size="small" type="success" @click="handlePostBind">
          <font-awesome-icon icon="fa-solid fa-link" />
        </el-button>
      </el-tooltip>
    </blockquote>
    <div id="post-detail-content" v-beauty v-highlight v-html="post.description"></div>

    <!-- ------------------------------------------------ -->

    <el-dialog v-model="postBindDialogVisible" :title="$t('service.tab.post.bind')">
