  ...
  const githubPagesMethods = {
    ...
    getMdFilename: (): string => {
      ...
      if (isEmptyString(mdFilenameRule)) {
        ...
        if (mdFilenameRule.indexOf("slug") > -1) {
          mdTitle = mdTitle.replace(/\[slug]/g, slugData.customSlug ?? slugPlace)
        }
        let date = new Date()
    ...
    initGithubPages: (paths: any, siyuanData?: SiyuanDataObj) => {
      ...
      if (siyuanData) {
        ...
        githubPagesData.usePermalink = parseBoolean(siyuanData.meta[usePermalinkKey] ?? "true")
        const useDateKey = SIYUAN_PAGE_ATTR_KEY.SIYUAN_PAGE_ATTR_CUSTOM_USE_DATE_KEY
        githubPagesData.useDate = parseBoolean(siyuanData.meta[useDateKey] ?? "false")
      }
    },
  }
